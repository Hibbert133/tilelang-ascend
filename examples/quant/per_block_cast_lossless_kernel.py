...
def get_per_block_cast_lossless_kernel(
    ...
    if in_config.use_tma_aligned_col_major_sf:
        x_sf_load_local_shape = (
            num_in_sf_per_block_k,
            num_in_sf_per_vector_m,
        )
    else:
        x_sf_load_local_shape = (
            num_in_sf_per_vector_m,
    ...
    def load_input_sf_exp_block(x_sf_exp_grouped_ub, x_sf_load_ub, x_sf, sf_m, sf_k):
        if in_config.use_packed_ue8m0:
            ...
            for flat in T.serial(num_out_sf_per_block * num_in_sf_per_out_sf):
                ...
                v = T.Cast(
                    "float32",
                    T.Cast("int32", x_sf_load_ub[in_j // 4, in_i * 4 + in_j % 4]),
                )
            ...
            for flat in T.serial(num_out_sf_per_block * num_in_sf_per_out_sf):
                ...
                in_i = out_i * num_in_sf_per_out_sf_m + reduce_i
                in_j = out_j * num_in_sf_per_out_sf_k + reduce_j
                sf_bits = T.reinterpret("int32", x_sf_load_ub[in_j, in_i])
                x_sf_exp_grouped_ub[group, inner] = T.Cast(
                    "float32",
                    (sf_bits >> 23) & 0xFF,
                )
        else:
            ...
            for flat in T.serial(num_out_sf_per_block * num_in_sf_per_out_sf):
                ...
                in_i = out_i * num_in_sf_per_out_sf_m + reduce_i
                in_j = out_j * num_in_sf_per_out_sf_k + reduce_j
                sf_bits = T.reinterpret("int32", x_sf_load_ub[in_i, in_j])
                x_sf_exp_grouped_ub[group, inner] = T.Cast(
                    "float32",
                    (sf_bits >> 23) & 0xFF,
                )
    ...
    def load_input_sf_exp_local(
        ...
        for flat in T.serial(local_num_out_sf_per_block * num_in_sf_per_out_sf):
            group = flat // num_in_sf_per_out_sf
            inner = flat % num_in_sf_per_out_sf
            out_i = group // num_out_sf_per_block_k
            out_j = group % num_out_sf_per_block_k
            ...
            if in_config.use_tma_aligned_col_major_sf:
                sf_value = x_sf_load_ub[in_j, in_i]
            else:
                sf_value = x_sf_load_ub[in_i, in_j]
    ...
    def reduce_output_sf_exp(
        ...
        T.reduce_max(
            ...
            dim=-1,
            clear=True,
            real_shape=[num_out_sf_per_block, num_in_sf_per_out_sf_aligned],
        )
        ...
        T.tile.cast(
            ...
            out_sf_exp_flat_ub,
            mode="CAST_ROUND",
            count=num_out_sf_per_block,
        )

    @T.macro
    def reduce_output_sf_exp_local(
        out_sf_exp_ub,
        x_sf_exp_grouped_ub,
        out_sf_exp_flat_ub,
    ):
        T.reduce_max(
            ...
            dim=-1,
            clear=True,
            real_shape=[local_num_out_sf_per_block, num_in_sf_per_out_sf_aligned],
        )
        ...
        T.tile.cast(
            ...
            out_sf_exp_flat_ub,
            mode="CAST_ROUND",
            count=local_num_out_sf_per_block,
        )

    @T.macro
    def compute_relative_sf(
        relative_sf_ub,
        x_sf_exp_grouped_ub,
        out_sf_exp_ub,
        sf_exp_row_offset,
    ):
        relative_exp_i32_ub = T.alloc_ub(
            (vector_data_tile_m * num_in_sf_per_block_k,),
            "int32",
        )
        relative_sf_bits_ub = T.alloc_ub(
            (vector_data_tile_m * num_in_sf_per_block_k,),
            "int32",
        )
        input_exp_i32_ub = T.alloc_ub(
            (vector_data_tile_m * num_in_sf_per_block_k,),
            "int32",
        )
        out_sf_exp_brd_ub = T.alloc_ub(
            (vector_data_tile_m * num_in_sf_per_block_k,),
            "int32",
        )
        for flat in T.serial(vector_data_tile_m * num_in_sf_per_block_k):
            ...
            input_exp_i32_ub[flat] = T.Cast(
                "int32",
                x_sf_exp_grouped_ub[group, inner],
            )
    ...
    def store_output_sf_local(out_sf, out_sf_fp32_ub, out_sf_row_offset, out_sf_col_offset):
        if not out_config.use_tma_aligned_col_major_sf:
            out_sf_store_ub = T.alloc_ub(
                (local_num_out_sf_per_block_m, num_out_sf_per_block_k),
                "float32",
            )
            ...
            out_sf_store_ub = T.alloc_ub(
                (num_out_sf_per_block_k, local_num_out_sf_per_block_m),
                "float32",
            )
            for flat in T.serial(local_num_out_sf_per_block):
                i = flat // num_out_sf_per_block_k
    ...
    def apply_relative_sf_tiles(relative_sf_ub, x, out, row_offset, col_offset):
        ...
        relative_sf_tile_ub = T.alloc_ub(
            (vector_data_tile_m, in_sf_block_k),
            "float32",
        )
        ...
        for sf_k in T.serial(num_in_sf_per_block_k):
            ...
            T.tile.cast(
                ...
                x_in_ub,
                mode="CAST_NONE",
                count=vector_data_tile_m * in_sf_block_k,
            )
    ...
    @T.macro
    def apply_relative_sf_tiles_local_direct(
        x_sf_load_ub,
        out_sf_fp32_ub,
        x,
        out,
    ...
    @T.macro
    def apply_relative_sf_tiles_pair_local_direct(
        x_sf_load_ub,
        out_sf_fp32_ub,
        x,
        out,
        ...
        relative_sf_tile0_ub = T.alloc_ub(
            (vector_data_tile_m, in_sf_block_k),
            "float32",
        )
        relative_sf_tile1_ub = T.alloc_ub(
            (vector_data_tile_m, in_sf_block_k),
            "float32",
        )
    ...
    @T.macro
    def apply_relative_sf_tiles_local_broadcast(
        x_sf_load_ub,
        out_sf_fp32_ub,
        x,
        out,
    ...
    @T.macro
    def apply_relative_sf_tiles_pair_local_exp(
        x_sf_exp_grouped_ub,
        out_sf_exp_ub,
        x,
        out,
        ...
        input_exp0_ub = T.alloc_ub((vector_data_tile_m, 1), "int32")
        input_exp1_ub = T.alloc_ub((vector_data_tile_m, 1), "int32")
        out_exp_ub = T.alloc_ub((vector_data_tile_m, 1), "int32")
        relative_exp0_ub = T.alloc_ub((vector_data_tile_m, 1), "int32")
        relative_exp1_ub = T.alloc_ub((vector_data_tile_m, 1), "int32")
        relative_bits0_ub = T.alloc_ub((vector_data_tile_m, 1), "int32")
        relative_bits1_ub = T.alloc_ub((vector_data_tile_m, 1), "int32")
        relative_sf_col0_ub = T.alloc_ub((vector_data_tile_m, 1), "float32")
        relative_sf_col1_ub = T.alloc_ub((vector_data_tile_m, 1), "float32")
        relative_sf_tile0_ub = T.alloc_ub(
            (vector_data_tile_m, in_sf_block_k),
        ...
        for out_sf_k in T.serial(num_out_sf_per_block_k):
            for row in T.serial(vector_data_tile_m):
                group_base = row * num_out_sf_per_block_k
                group = group_base + out_sf_k
                out_exp_ub[row, 0] = out_sf_exp_ub[group]
            T.tile.add(out_exp_ub, out_exp_ub, -127)
            ...
            for reduce_pair in T.serial(num_in_sf_per_out_sf_k // 2):
                ...
                for row in T.serial(vector_data_tile_m):
                    group_base = row * num_out_sf_per_block_k
                    group = group_base + out_sf_k
                    input_exp0_ub[row, 0] = T.Cast(
                        "int32",
                        x_sf_exp_grouped_ub[group, reduce_j0],
                    )
                    input_exp1_ub[row, 0] = T.Cast(
                        "int32",
                        x_sf_exp_grouped_ub[group, reduce_j1],
                    )

                T.tile.sub(relative_exp0_ub, input_exp0_ub, out_exp_ub)
                T.tile.sub(relative_exp1_ub, input_exp1_ub, out_exp_ub)
                T.tile.bitwise_lshift(relative_bits0_ub, relative_exp0_ub, 23)
                T.tile.bitwise_lshift(relative_bits1_ub, relative_exp1_ub, 23)
                T.reinterpretcast(relative_sf_col0_ub, relative_bits0_ub, "float")
                T.reinterpretcast(relative_sf_col1_ub, relative_bits1_ub, "float")
                T.tile.broadcast(relative_sf_tile0_ub, relative_sf_col0_ub, axis=1)
                T.tile.broadcast(relative_sf_tile1_ub, relative_sf_col1_ub, axis=1)
            ...
            if num_in_sf_per_out_sf_k % 2 != 0:
                ...
                for row in T.serial(vector_data_tile_m):
                    ...
                    input_exp0_ub[row, 0] = T.Cast(
                        "int32",
                        x_sf_exp_grouped_ub[group, reduce_j],
                    )
    ...
    @T.macro
    def apply_relative_sf_tiles_pair_local_exp_tile(
        x_sf_exp_grouped_ub,
        out_sf_exp_ub,
        x,
        out,
        ...
        with T.Kernel(m_num * n_num, is_npu=True) as (cid, vid):
            with T.Scope("V"):
                ...
                if use_vid_local_sf:
                    ...
                    local_out_sf_row_offset = (
                        out_sf_row_offset + vid * local_num_out_sf_per_block_m
                    )
                    x_sf_load_ub = T.alloc_ub(x_sf_load_local_shape, "float32")
                    x_sf_exp_grouped_ub = T.alloc_ub(
                        (local_num_out_sf_per_block, num_in_sf_per_out_sf_aligned),
                        "float32",
                    )
                    out_sf_exp_ub = T.alloc_ub(
                        (local_num_out_sf_per_block,),
                        "int32",
                    )
                    ...
                    if (
                        ...
                        apply_relative_sf_tiles_local_broadcast(
                            x_sf_load_ub,
                            out_sf_fp32_ub,
                            x,
                            out,
                        ...
                        apply_relative_sf_tiles_pair_local_direct(
                            x_sf_load_ub,
                            out_sf_fp32_ub,
                            x,
                            out,
                        ...
                        apply_relative_sf_tiles_local_direct(
                            x_sf_load_ub,
                            out_sf_fp32_ub,
                            x,
                            out,
                    ...
                    relative_sf_ub = T.alloc_ub(
                        (vector_data_tile_m * num_in_sf_per_block_k,),
                        "float32",
                    )
                    x_sf_load_ub = T.alloc_ub(x_sf_load_shape, x_sf_load_dtype)
                    x_sf_exp_grouped_ub = T.alloc_ub(
                        (num_out_sf_per_block, num_in_sf_per_out_sf_aligned),
                        "float32",
                    )
                    out_sf_exp_ub = T.alloc_ub(
                        (num_out_sf_per_block,),
                        "int32",
                    )
                    out_sf_fp32_ub = T.alloc_ub(
                        (num_out_sf_per_block,),
                            ...
                            for i in T.serial(num_out_sf_per_block_m):
                                for j in T.serial(num_out_sf_per_block_k):
                                    ...
                                    if out_config.use_packed_ue8m0:
                                        store_sf(
                                            out_sf,
                                            T.Cast("uint8", out_sf_exp_ub[group]),
                                            out_sf_row_offset + i,
                                            out_sf_col_offset + j,
                                            out_config,
                                        )
                    ...
                    compute_relative_sf(
                        ...
                        x_sf_exp_grouped_ub,
                        out_sf_exp_ub,
                        vid * vector_data_tile_m // in_sf_block_m,
                    )
